...
$lengui = array(
    ...
    'remove_notice' => array ('en' => 'Your risk has been deleted.', 'lt' => 'Jūsų rizika buvo pašalinta.', 'pl' => 'Wasze kontrol bylo usuwane.'),
    'create_notice' => array ('en' => 'Your risk has been created.', 'lt' => 'Jūsų rizika buvo pridėta.', 'pl' => 'Wasze kontrol bylo prowadzone.'),
    'logout' => array ('en' => 'Log Out', 'lt' => 'Atsijungti', 'pl' => 'Wyloguj'),
    'risk' => array ('en' => 'Controls', 'lt' => 'Kontrolės', 'pl' => 'Kontroli'),
    ...
    'filter_type1' => array ('en' => 'Risk type', 'lt' => 'Rizikos rūšis', 'pl' => 'Rodzaj ryzyki'),
    'filter_type2' => array ('en' => 'Risk name', 'lt' => 'Rizikos pavadinimas', 'pl' => 'Nazwa ryzyki'),
    'filter_type3' => array ('en' => 'Risk owner', 'lt' => 'Rizikos savininkas', 'pl' => 'Właściciel ryzyki'),
    'filter_search' => array ('en' => 'Initiate', 'lt' => 'Inicijuoti', 'pl' => 'Inicjować'),
    'filter_clear' => array ('en' => 'Empty it', 'lt' => 'Ištuštinti', 'pl' => 'Puści'),
    'reduce_risk' => array ('en' => 'Reduce this risk', 'lt' => 'Slopinti riziką', 'pl' => 'Redukować ten ryzyk'),
    'following_info' => array ('en' => 'Total level of risk', 'lt' => 'Rizikos lygis iš viso', 'pl' => 'Level ryzyku w totale'),
...
if(isset($_GET["tabel"])) {
	if($_GET["tabel"] == 1) {
	$filename = "tabel-rsk-".date("Ymd").".xlsx";
	...
	$keywords = array($lengui['isms'][$current],$lengui['risk'][$current]);
	$writer->setTitle($lengui['risk'][$current]);
	$writer->setSubject($lengui['risk'][$current]);
	...
	$header = array("string","integer","string","string","integer","integer","string","string","integer","string");
		
	$selectqry = "SELECT `risk_name`,`risk_type`, `threat_exp`, `vulnerability_exp`, `impact_v`, `likelihood_v`, `risk_level_p`, `risk_owner_name`, `treatment_decision`, `treatment_plan` from `risk_register`;";
	
	if($result=mysqli_query($connection,$selectqry)) {
		while ($row = mysqli_fetch_assoc($result)) {
			$risk_level_p = ($row["risk_level_p"] == '+')? $lengui['less-important'][$current] : $lengui['usual'][$current] ;
			$rows[] = [$row["risk_name"],$row["risk_type"],$row["threat_exp"],$row["vulnerability_exp"],$row["impact_v"],$row["likelihood_v"],$risk_level_p,$row["risk_owner_name"],$row["treatment_decision"],$row["treatment_plan"]];
		}
	...
	$writer->markMergedCell($sheet1, $start_row=0, $start_col=0, $end_row=0, $end_col=9);
	...
	<nav class="navbar navbar-inverse">
		<div class="container-fluid">
			...
			<ul class="nav navbar-nav">
				...
				<li><a href="riskregister.php?tabel=1"><?php echo $lengui['nav_title_tabel'][$current]; ?></a></li>
			</ul>
...
if(isset($_POST['btnSearch'])) {
    ...
    if($control == $COOKIE_SECURITY) {
        if($type == 1) {//risk_type
        if(strpos($name,'asset') !== false || strpos($name,'worth') !== false || strpos($name,'vertybė') !== false || strpos($name,'asset') !== false || strpos($name,'wartość') !== false) { $type_v[] = 1; }
        if(strpos($name,'process') !== false || strpos($name,'procesas') !== false || strpos($name,'proces') !== false) { $type_v[] = 2; }
        if(strpos($name,'system') !== false || strpos($name,'sistema') !== false) { $type_v[] = 3; }
        ...
        switch ($siz) {
            ...
            case 1:
                ...
                break;
            case 2:
                $src = "WHERE risk_type = ? OR risk_type = ?";
                $tip = "B2";
                break;
            case 3:
                $src = "WHERE risk_type = ? OR risk_type = ? OR risk_type = ?";
                $tip = "B3";
                break;
        }
        $_SESSION['filter'] = array('type' => 1, 'name' => $name);
        } elseif($type == 2) {//risk_name
            $src = "WHERE risk_name LIKE ?";
            $tip = "C";
            $_SESSION['filter'] = array('type' => 2, 'name' => $name);
        } elseif($type == 3) {//risk_owner
            $src = "WHERE risk_owner_name LIKE ?";
            $tip = "D";
...
<!-- Filter -->
					<tr class="view-row">
						<td class="view-cell view-head">
							...
							<form method="POST" action="riskregister.php">
...
<?php
    $selectqry = "SELECT * FROM risk_register {$src};";
    ...
    $getQuery = "SELECT *, IF(risk_level_p='*',impact_v * likelihood_v,IF(risk_level_p='+',impact_v + likelihood_v,impact_v + likelihood_v)) AS risk_level, (SELECT SUM(risk_change) FROM implemented_controls_a WHERE risk_register_un = risk_register.risk_un) AS risk_change FROM risk_register {$src} ORDER BY risk_level+risk_change DESC LIMIT " . $initial_page . ',' . $limit;
    ...
    switch ($tip) {
        ...
        case "B1":
            ...
            mysqli_stmt_bind_param($stmt2,'i', $type_v[0]);
            mysqli_stmt_execute($stmt2);
            $resultLimit = mysqli_stmt_get_result($stmt2);
            break;
        case "B2":
            $stmt2 = mysqli_prepare($connection, $getQuery);
            mysqli_stmt_bind_param($stmt2,'ii', $type_v[0], $type_v[1]);
            mysqli_stmt_execute($stmt2);
            $resultLimit = mysqli_stmt_get_result($stmt2);
            break;
        case "B3":
            $stmt2 = mysqli_prepare($connection, $getQuery);
            mysqli_stmt_bind_param($stmt2,'iii', $type_v[0], $type_v[1], $type_v[2]);
            mysqli_stmt_execute($stmt2);
            $resultLimit = mysqli_stmt_get_result($stmt2);
            break;
         case "C":
...
<!-- Brief view -->
					<tr class="view-row">
						<td class="view-cell view-head">
							<p><?php echo $lengui['following_info'][$current]; ?> <?php echo htmlentities($isms_total_risk_level, ENT_QUOTES, 'UTF-8'); ?></p>
							<form method="POST" action="controls.riskregister.php">
								<input type="hidden" name="isms_un" value="<?php echo $isms_un; ?>">
								<input type="hidden" name="manage_sec" value="<?php echo $COOKIE_SECURITY; ?>">							
								<button type="submit" name="reduce" class="edit-buttons"><img src="images/reduce.png" class="edit-icons"></button>
							</form>
							<form method="POST" action="manage.riskregister.php">
								<input type="hidden" name="isms_un" value="<?php echo $isms_un; ?>">
								<input type="hidden" name="manage_sec" value="<?php echo $COOKIE_SECURITY; ?>">							
...
<?php
				...
				{  
?>
					<a href = "riskregister.php?page=<?php echo $page_number; ?>" class="pagenumbers"><?php echo $page_number; ?></a> 	
<?php
				}
?>
			</div>
